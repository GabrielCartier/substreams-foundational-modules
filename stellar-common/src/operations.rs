use crate::{
    pb::sf::{
        ...
        substreams::stellar::r#type::v1::{
            operation::Op, AccountMerge, AllowTrust, ClaimClaimableBalance, Clawback, ClawbackClaimableBalance,
            CreateAccount, CreateClaimableBalance, CreatePassiveSellOffer, LiquidityPoolDeposit, LiquidityPoolWithdraw,
            ManageBuyOffer, ManageSellOffer, Operation, Operations, PathPaymentStrictReceive, PathPaymentStrictSend,
            Payment, SetTrustLineFlags,
        },
    },
    utils,
};
...
fn map_operations(block: Block) -> Result<Operations, substreams::errors::Error> {
            ...
            stellar_xdr::curr::OperationBody::LiquidityPoolDeposit(liquidity_pool_deposit) => {
                operations.operations.push(Operation {
                    op: Some(Op::LiquidityPoolDeposit(LiquidityPoolDeposit {
                        trx_hash: hash.clone(),
                        ledger_sequence: block.number,
                        liquidity_pool_id: liquidity_pool_deposit.liquidity_pool_id.0.to_string(),
            ...
            stellar_xdr::curr::OperationBody::LiquidityPoolWithdraw(liquidity_pool_withdraw) => {
                operations.operations.push(Operation {
                    op: Some(Op::LiquidityPoolWithdraw(LiquidityPoolWithdraw {
                        trx_hash: hash.clone(),
                        ledger_sequence: block.number,
                        liquidity_pool_id: liquidity_pool_withdraw.liquidity_pool_id.0.to_string(),
            ...
            stellar_xdr::curr::OperationBody::PathPaymentStrictSend(path_payment_strict_send) => {
                operations.operations.push(Operation {
                    op: Some(Op::PathPaymentStrictSend(PathPaymentStrictSend {
                        trx_hash: hash.clone(),
                        ledger_sequence: block.number,
                        send_asset: Some(utils::create_asset(&path_payment_strict_send.send_asset)),
                        send_amount: path_payment_strict_send.send_amount,
                        destination: path_payment_strict_send.destination.to_string(),
                        dest_asset: Some(utils::create_asset(&path_payment_strict_send.dest_asset)),
            ...
            stellar_xdr::curr::OperationBody::PathPaymentStrictReceive(path_payment_strict_receive) => {
                operations.operations.push(Operation {
                    op: Some(Op::PathPaymentStrictReceive(PathPaymentStrictReceive {
                        trx_hash: hash.clone(),
                        ledger_sequence: block.number,
                        send_asset: Some(utils::create_asset(&path_payment_strict_receive.send_asset)),
                        send_max: path_payment_strict_receive.send_max,
                        destination: path_payment_strict_receive.destination.to_string(),
                        dest_asset: Some(utils::create_asset(&path_payment_strict_receive.dest_asset)),
